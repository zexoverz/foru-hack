...
contract DIDManager is AccessControl {
    ...
    // Add mapping to track if address already has a DID
    ...
    event CommunityDIDMinted(
        uint256 indexed tokenId,
        address indexed owner,
        string name
    );
    event CommunityFused(
        uint256 indexed userTokenId,
        uint256 indexed communityTokenId,
        uint256 timestamp
    );
    event CommunityLeft(
        uint256 indexed userTokenId,
    ...
    function mintUserDID(
        ...
        require(!hasDID[to], "Address already has a DID");

        user.mint(tokenId, to);
        userPublicKeys[tokenId] = publicKey;

        // Track the DID for this address
        addressToDID[to] = tokenId;
        hasDID[to] = true;
    ...
    // Add getter function
    function getDIDForAddress(address user) external view returns (uint256) {
        require(hasDID[user], "Address has no DID");
        return addressToDID[user];
    }

    function addressHasDID(address user) external view returns (bool) {
        return hasDID[user];
    }
     ...
     */
    function fuseWithCommunity(
        uint256 userTokenId,
        uint256 communityTokenId
    ) external {
        // Verify user owns the user DID token
        require(
            user.ownerOf(userTokenId) == msg.sender,
            "Not owner of user DID"
        );

        // Verify community DID exists
        require(
            community.ownerOf(communityTokenId) != address(0),
            "Community DID does not exist"
        );
        ...
        require(
            _meetsRequirements(userTokenId, communityTokenId),
    ...
    function addBadgeToUser(
        ...
        require(
            user.ownerOf(userTokenId) != address(0),
    ...
    function addXPToUser(
        ...
        emit UserProfileUpdated(
            userTokenId,
            userProfiles[userTokenId].persona,
            userProfiles[userTokenId].xp
        );
    }
    ...
    function updateCommunityRequirements(
        ...
        require(
            community.ownerOf(communityTokenId) == msg.sender,
            "Not community owner"
        );
    ...
    function estimateGasForFusion(
        ...
        try community.ownerOf(communityTokenId) returns (
     ...
     */
    function getCommunityRequirements(
        uint256 communityTokenId
    )
        external
        view
        returns (uint256 minXp, string[] memory requiredBadges, bool isActive)
    {
        CommunityRequirements memory requirements = communityRequirements[
            communityTokenId
        ];
        return (
            requirements.minXp,
    ...
    function _meetsRequirements(
        ...
        for (uint256 i = 0; i < requirements.requiredBadges.length; i++) {
            ...
            for (uint256 j = 0; j < profile.badges.length; j++) {
                if (
                    keccak256(bytes(requirements.requiredBadges[i])) ==
                    keccak256(bytes(profile.badges[j]))
                ) {
                    hasBadge = true;
                    break;
                }
